...
const PERM_CATEGORIES = [
  { key: 'dashboard', label: '메인', perms: ['view_all'] },
  { key: 'applications', label: '신청 관리', perms: ['approve', 'reject', 'create', 'edit', 'delete'] },
  { key: 'account', label: '계정 관리', perms: ['account_access', 'manage_accounts'] },
  { key: 'operations', label: '운영', perms: ['view_revenue'] },
  { key: 'site', label: '홈페이지 관리', perms: ['system_settings', 'data_management'] },
  { key: 'special', label: '특수 권한', perms: ['create_super_admin', 'supabase_access'] },
];
...
const AccountsPage = () => {
  ...
  const [matrixUser, setMatrixUser] = useState<any>(null);
  ...
  const handleUpdateUserPerm = async (targetUser: any, permKey: string, value: boolean) => {
    if (!can('manage_accounts')) return;
    const nextOverrides = {
      ...(targetUser.perm_overrides || {}),
      [permKey]: value
    };

    const { error } = await DB.updateAdminProfile(targetUser.id, {
      perm_overrides: nextOverrides,
      updated_at: new Date().toISOString()
    });

    if (!error) {
      setProfiles((prev) => prev.map((item) => item.id === targetUser.id ? { ...item, perm_overrides: nextOverrides } : item));
      setMatrixUser((prev: any) => prev && prev.id === targetUser.id ? { ...prev, perm_overrides: nextOverrides } : prev);
      await DB.createAdminLog({
        type: 'sliders',
        color: 'var(--lav)',
        title: '개별 권한 수정',
        desc: `'${targetUser.name}' 계정의 '${PERM_LABELS[permKey] || permKey}' 권한이 ${value ? '활성화' : '비활성화'}되었습니다.`,
        created_at: new Date().toISOString()
      });
    } else {
      alert('권한 저장 실패: ' + (error as any).message);
    }
  };
  ...
  const filteredData = profiles.filter(p => {
    ...
    const canSeeRow = can('manage_accounts')
      ? isVisibleByRank
      : currentUserRole === 'operator'
        ? isSelf
        : isSelf || ROLE_RANK[p.role] < ROLE_RANK[currentUserRole];
    const isVisibleByFilter = filter === 'all' || p.role === filter;
    return canSeeRow && isVisibleByFilter;
  });

  const getRoleCounts = () => {
    const counts: any = { master_admin: 0, super_admin: 0, admin: 0, operator: 0 };
    profiles.forEach(p => { if (counts[p.role] !== undefined) counts[p.role]++; });
    return counts;
  };
  const counts = getRoleCounts();
  const visibleRoleKeys = Object.keys(dynamicRoles).filter((key) => ROLE_RANK[key] <= ROLE_RANK[currentUserRole]);
  const editableRoleKeys = Object.keys(dynamicRoles).filter((key) => ROLE_RANK[key] < ROLE_RANK[currentUserRole]);
  const roleMatrixPerms = Object.entries(PERM_LABELS).filter(([permKey]) => !['create_super_admin', 'supabase_access'].includes(permKey));
    ...
    <div className="space-y-6">
      <div className="acc-level-grid">
        {Object.entries(dynamicRoles)
          ...
          <div 
            key={key} 
            className={`account-role-card ${filter === key ? 'on' : ''}`}
            onClick={() => setFilter(key)}
            style={{ 
              ['--role-accent' as any]: r.color
            }}
          >
            <div className="account-role-top">
              <div className="account-role-icon">
                <i className={`fa-solid ${r.icon}`}></i>
              </div>
              <div className="account-role-meta">
                <div className="account-role-name">{r.label}</div>
                <div className="account-role-desc">{r.label} 권한을 가진 계정 수</div>
              </div>
              <div className="account-role-arrow">
                <i className="fa-solid fa-arrow-right"></i>
              </div>
            </div>
            <div className="account-role-count">{counts[key] || 0}</div>
            <div className="account-role-foot">
              <span className="account-role-chip">권한 그룹</span>
              <span className="account-role-filter">{filter === key ? '선택됨' : '클릭하여 보기'}</span>
            </div>
          </div>
        ))}
      </div>
            ...
            <table>
              ...
              <tbody>
                {loading ? (
                  ...
                  const cannotEditProfile = isSelf ? false : (!isPeerSuperAdmin && ROLE_RANK[u.role] >= ROLE_RANK[currentUserRole]);
                  ...
                  <table style={{ width: '100%' }}>
                    ...
                    <tbody>
                      {PERM_CATEGORIES.map((category) => {
                        const categoryPerms = roleMatrixPerms.filter(([permKey]) => category.perms.includes(permKey));
                        if (categoryPerms.length === 0) return null;
                        return (
                          <React.Fragment key={category.key}>
                            <tr style={{ borderTop: '1px solid var(--line)' }}>
                              <td colSpan={editableRoleKeys.length + 1} style={{ padding: '10px 0 6px', color: 'var(--head)', fontSize: '.68rem', fontWeight: 800, letterSpacing: '.08em' }}>
                                {category.label}
                              </td>
                            </tr>
                            {categoryPerms.map(([permKey, label]) => (
      ...
      {matrixUser && (
        <div className="modal-wrap on" onClick={(e) => e.target === e.currentTarget && setMatrixUser(null)}>
          <div className="modal" style={{ maxWidth: '760px' }}>
            <div className="modal-head">
              <span className="modal-ttl">개별 권한 매트릭스 ({matrixUser.name})</span>
              <button className="modal-close" onClick={() => setMatrixUser(null)}><i className="fa-solid fa-times"></i></button>
            </div>
            <div className="modal-body" style={{ display: 'grid', gap: '14px' }}>
              {PERM_CATEGORIES.map((category) => {
                const categoryPerms = Object.entries(PERM_LABELS).filter(([permKey]) => category.perms.includes(permKey));
                if (categoryPerms.length === 0) return null;
                return (
                  <div key={category.key} className="card" style={{ marginBottom: 0 }}>
                    <div className="card-h">
                      <span className="card-title">{category.label}</span>
                    </div>
                    <div className="card-body" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '10px' }}>
                      {categoryPerms.map(([permKey, label]) => {
                        const baseChecked = !!(dynamicRoles[matrixUser.role]?.perms?.[permKey]);
                        const overrideChecked =
                          typeof matrixUser?.perm_overrides?.[permKey] === 'boolean'
                            ? matrixUser.perm_overrides[permKey]
                            : baseChecked;
                        const isLockedSpecial = ['create_super_admin', 'supabase_access'].includes(permKey);
                        return (
                          <label key={permKey} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', padding: '10px 12px', border: '1px solid var(--line)', borderRadius: '10px', background: 'var(--ink3)' }}>
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontSize: '.76rem', fontWeight: 700, color: 'var(--head)' }}>{label}</div>
                              <div style={{ fontSize: '.64rem', color: 'var(--muted)', marginTop: '3px' }}>
                                기본값: {baseChecked ? '허용' : '제한'}
                              </div>
                            </div>
                            <input
                              type="checkbox"
                              checked={overrideChecked}
                              disabled={isLockedSpecial}
                              onChange={(e) => handleUpdateUserPerm(matrixUser, permKey, e.target.checked)}
                            />
                          </label>
                        );
                      })}
            ...
            <div className="modal-foot">
              <button className="btn" onClick={() => setMatrixUser(null)}>닫기</button>
            </div>
          </div>
        </div>
      )}
