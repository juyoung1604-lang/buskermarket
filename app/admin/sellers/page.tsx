...
const getInquiryMessage = (note?: string) => {
  const message = note?.split('[SNS/링크]')[0].replace('[문의]', '').trim() || '';
  if (!message || message.startsWith('거절 사유:')) return '내용 없음';
  return message;
};
...
const SellersPage = () => {
  ...
  const handleBulkStatusChange = async (status: string) => {
    ...
    if (status === 'pool') {
      ...
      for (const id of selectedIds) {
        ...
        if (item) await DB.transferApplicantToPool('seller', item);
      }
  ...
  const handleStatusChange = async (id: string, status: string) => {
    ...
    const { error } = await DB.updateStatus('sellers', id, status, payload);
    if (!error) {
      setSellers(prev => prev.map(s => s.id === id ? { ...s, status, ...payload } : s));
      toast('처리되었습니다.', 'jade');
    }
  };
