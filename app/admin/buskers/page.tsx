...
const getInquiryMessage = (note?: string) =>
  note?.split('[SNS/링크]')[0].replace('[문의]', '').trim() || '내용 없음';

const getRejectionReason = (item: any) => item?.rejection_reason?.trim() || '';
...
const BuskersPage = () => {
  ...
  const handleBulkStatusChange = async (status: string) => {
    ...
    if (status === 'pool') {
      ...
      for (const id of selectedIds) {
        ...
        if (item) await DB.transferApplicantToPool('busker', item);
      }
      toast(`${selectedIds.length}건의 항목이 통합 인력 풀 DB로 이관되었습니다.`, 'jade');
      setSelectedIds([]);
      fetchData();
      return;
    }
    ...
    const rejectionReasonInput =
      status === 'rejected'
        ? window.prompt('거절 사유를 입력하세요. 입력한 사유는 선택한 항목에 동일하게 저장됩니다.', '')
        : '';

    if (status === 'rejected' && rejectionReasonInput === null) {
      setLoading(false);
      return;
    }
  ...
  const handleStatusChange = async (id: string, status: string) => {
    ...
    if (status === 'rejected' && rejectionReasonInput === null) return;
    ...
    const { error } = await DB.updateStatus('buskers', id, status, payload);
    if (!error) {
      setBuskers(prev => prev.map(b => b.id === id ? { ...b, status, ...payload } : b));
      toast('처리되었습니다.', 'jade');
    }
  };
                  ...
                  <td>
                    <div className="td-acts">
                      ...
                      <button className="ico-btn" onClick={async () => {
                        await DB.transferApplicantToPool('busker', b);
      ...
      {selectedItem && (
        <DetailModal isOpen={!!selectedItem} onClose={() => setSelectedItem(null)} title={`버스커 상세 — ${selectedItem.name}`} data={[
          ...
          ...(selectedItem.status === 'rejected'
            ? [{ label: '거절 사유', value: getRejectionReason(selectedItem) || '미입력' }]
            : []),
          { label: '이력', value: checkDuplicate(selectedItem.phone, selectedItem.email) ? '기존 활동자' : '신규 신청자' },
          { label: '문의 내용', value: <div style={{ whiteSpace: 'pre-wrap', fontSize: '.85rem', color: 'var(--head)' }}>{getInquiryMessage(selectedItem.note)}</div> },
          { label: 'SNS / 링크', value: (
