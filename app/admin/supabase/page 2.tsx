...
const SupabasePage = () => {
  ...
  const handleSeedData = async () => {
    if (!confirm('실제 데이터베이스의 기존 샘플/운영 데이터를 비우고 샘플 데이터를 다시 채울까요?')) return;
    try {
      await DB.seedData();
  ...
  const handleClearSampleData = async () => {
    if (!confirm('실제 데이터베이스의 샘플 데이터를 삭제할까요? 이 작업은 되돌릴 수 없습니다.')) return;
    try {
      await DB.clearSampleData();
      toast('샘플 데이터 삭제 완료! 페이지를 새로고침하세요.', 'jade');
    } catch (e: any) {
      toast('삭제 실패: ' + e.message, 'rose');
    }
  };
          ...
          <div className="card">
            ...
            <div className="card-body">
              <div className="space-y-4">
                ...
                <button className="btn btn-rose" style={{ width: '100%', justifyContent: 'center', marginTop: '8px' }} onClick={handleClearSampleData}>
                  <i className="fa-solid fa-trash"></i> DB 샘플 데이터 삭제
                </button>
              </div>
            </div>
          </div>
