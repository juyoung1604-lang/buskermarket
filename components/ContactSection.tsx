...
export default function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.privacyAgreed) {
      toast("개인정보 수집 및 이용에 동의해주세요.", "rose");
      return;
    }
    ...
    try {
      ...
      if (formData.type === 'busker') {
        ...
        res = await DB.createBusker(payload);
        // Add to pool automatically
        await DB.addToPool('busker', {
          name: payload.name,
          team: payload.team,
          genre: payload.genre,
          phone: payload.phone,
        ...
        res = await DB.createSeller(payload);
        // Add to pool automatically
        await DB.addToPool('seller', {
          name: payload.name,
      ...
      setFormData({
        ...
        type: "busker", team: "", genre: "어쿠스틱", category: "핸드메이드 공예", 
        booths: "1", date: "", message: "", privacyAgreed: false
      });
      setLinks([""]);
    } catch (err: any) {
        ...
        <AnimateOnScroll direction="up" delay={0.1}>
          <form onSubmit={handleSubmit} className="bg-white p-5 sm:p-8 md:p-12 rounded-[32px] shadow-xl border border-gray-100 space-y-6 md:space-y-8">
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div>
                <label style={labelStyle} className="block text-sm mb-2">생년월일 *</label>
                <input name="birth_date" required className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#FF8B5A] transition-all" type="date" value={formData.birth_date} onChange={handleChange} />
              </div>
              ...
              <div>
                <label style={labelStyle} className="block text-sm mb-2">소속 (팀명/회사 등)</label>
                <input name="organization" className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#FF8B5A] transition-all" type="text" value={formData.organization} onChange={handleChange} placeholder="소속이 없다면 '개인' 입력" />
              </div>
            ...
            <div className="md:col-span-2 bg-gray-50 p-6 rounded-2xl border border-gray-100">
              <h4 style={labelStyle} className="text-sm mb-4 flex items-center gap-2">
                <i className="ri-shield-check-fill text-[#FF8B5A]"></i> 개인정보 수집·이용·제공 동의
              </h4>
              <div className="text-xs text-gray-500 space-y-2 mb-6 bg-white p-4 rounded-xl border border-gray-100 max-h-40 overflow-y-auto">
                ...
                <p>1년</p>
                <p className="mt-3 text-[#FF8B5A] font-medium">※ 개인정보 수집, 이용 제공 동의를 거부할 수 있으며, 거부 시 이용이 제한될 수 있습니다.</p>
                <hr className="my-3 border-gray-100" />
                <p className="text-gray-700 leading-relaxed">
                  본인은 위의 정보를 사실에 근거하여 정확히 기입하였으며, 공연 중 발생할 수 있는 모든 책임은 신청자에게 있음을 인지하고 동의합니다.
                </p>
                <p className="text-right mt-4 font-bold text-gray-400">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className="relative flex items-center">
                  <input 
                    ...
                    className="w-6 h-6 rounded-lg border-2 border-gray-200 text-[#FF8B5A] focus:ring-[#FF8B5A] cursor-pointer transition-all checked:border-[#FF8B5A]" 
                  />
                </div>
                <span className="text-sm font-bold text-gray-700 group-hover:text-[#FF8B5A] transition-colors">
                  위의 내용을 확인하였으며, 개인정보 수집 및 이용에 동의합니다. *
                </span>
              </label>
            </div>
