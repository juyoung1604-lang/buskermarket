...
const AddModal = ({ type, isOpen, onClose, onSuccess, initialData, mode = 'default' }: AddModalProps) => {
    ...
    if (isOpen) {
      ...
      if (initialData) {
        ...
        setFormData({
          ...
          'f-birth': initialData.birth_date,
          'f-org': initialData.organization,
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (type === 'event') {
        ...
        if (type === 'busker') {
          const payload = {
            ...
            birth_date: formData['f-birth'],
            organization: formData['f-org'],
            event_date: formData['f-date'],
          ...
          if (isPoolMode) {
            res = await DB.updatePool('busker_pool', initialData.id, {
              name: payload.name,
              team: payload.team,
              genre: payload.genre,
              phone: payload.phone,
          ...
          if (isPoolMode) {
            res = await DB.updatePool('seller_pool', initialData.id, {
              name: payload.name,
      ...
      const resultData = res && typeof res === 'object' && 'data' in res ? (res as any).data : undefined;
          ...
          {type === 'event' ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="fg">
                  <label>생년월일 *</label>
                  <input className="fi" id="f-birth" type="date" required value={formData['f-birth'] || ''} onChange={handleChange} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="fg">
                  <label>소속 (팀명/회사 등)</label>
                  <input className="fi" id="f-org" value={formData['f-org'] || ''} onChange={handleChange} placeholder="개인/팀명 등" />
                </div>
              </div>
