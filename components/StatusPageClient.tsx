              ...
              <>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  {result.status === 'rejected' && (
                    <div className="rounded-2xl bg-[#FFF1F2] p-4 md:col-span-2">
                      <p className="mb-1 text-xs font-bold uppercase tracking-widest text-[#E11D48]">거절 사유</p>
                      <p className="text-sm font-bold text-[#881337]">{result.rejection_reason || '별도 안내 예정'}</p>
                    </div>
                  )}
                </div>
              </>
        ...
        <div className="mt-6 rounded-[28px] border border-[#FED7AA] bg-[#FFF7ED] p-6 shadow-sm md:p-8">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl bg-white/70 p-4">
              ...
              <p className="text-lg font-black text-[#7C2D12]">{settings.deposit_bank || '관리자 설정 필요'}</p>
            </div>
            <div className="rounded-2xl bg-white/70 p-4">
              <p className="mb-1 text-xs font-bold uppercase tracking-widest text-[#9A3412]">계좌번호</p>
              <p className="text-lg font-black text-[#7C2D12]">{settings.deposit_account || '관리자 설정 필요'}</p>
            </div>
          </div>
          <p className="mt-3 text-sm font-medium text-[#9A3412]">
            {settings.deposit_bank ? '입금 시 신청자명과 동일하게 보내주시면 확인이 빠릅니다.' : '입금 계좌 정보가 설정되지 않았습니다. 관리자에게 문의하세요.'}
          </p>
        </div>
      </div>
    </main>
